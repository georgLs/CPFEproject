...
b = zeros(3,m);
k = zeros(1,m);
actualCompany = 600009;

%price data
P = str2double(table2array(readtable('pricesReady.xlsx')));
Prices = [Companies; P];

%call function which calculates average
companyAverage = priceAverage(Prices,actualCompany);

%priceAverage = priceAverage(Prices, 4);
